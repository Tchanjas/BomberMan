        ...
        for (int i = 1; i <= expRadius; i++) {
            if (x + 1 - i > 0) {
                if (board.getMatrixItem(x - i, y).isDestructible()) {
                    ...
                    } else if (board.getMatrixItem(x - i, y).getClass().equals(new Enemy(x, y, board).getClass())) {
            ...
            if (y - 1 + i < board.getMatrix()[0].length - 1) {
                if (board.getMatrixItem(x, y + i).isDestructible()) {
                    ...
                    else if (board.getMatrixItem(x, y + i).getClass().equals(new Enemy(x, y, board).getClass())) {
                        board.setNumBombs(board.getNumBombs() + 1);
                    }
